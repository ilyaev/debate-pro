...
export function Waveform({ userAnalyserRef, aiAnalyserRef, status }: Props) {
    ...
    useEffect(() => {
        ...
        function draw() {
            ...
            const userDataArray = new Float32Array(bufferLength);
            const aiDataArray = new Float32Array(bufferLength);

            if (userAnalyser) userAnalyser.getFloatTimeDomainData(userDataArray);
            if (aiAnalyser) aiAnalyser.getFloatTimeDomainData(aiDataArray);
